...
public class QnAList extends HttpServlet {
	...
	protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		// 1.5 페이징
		totalCount = dao.getPersonalTotalCount(map); //총 게시물 수
		...
		if (n == 1) {
			...
			pagebar += String.format("<li>"
					+ "            <a href=\"/bookjuck/member/qna/list.do?page=%d\" aria-label=\"Previous\">"
					+ "                <span aria-hidden=\"true\">&laquo;</span>"
					+ "            </a>"
					+ "        </li>", n - 1);
		}
		...
		while (!(loop > blockSize || n > totalPage)) {
			...
			pagebar += String.format("<a href=\"/bookjuck/member/qna/list.do?page=%d\">%d</a></li>", n, n);
		...
		if (n > totalPage) {
			...
			pagebar += String.format("<li>"
					+ "            <a href=\"/bookjuck/member/qna/list.do?page=%d\" aria-label=\"Next\">"
					+ "                <span aria-hidden=\"true\">&raquo;</span>"
					+ "            </a>"
					+ "        </li>", n);
		}
		...
		RequestDispatcher dispatcher = req.getRequestDispatcher("/WEB-INF/views/member/qna/list.jsp");
